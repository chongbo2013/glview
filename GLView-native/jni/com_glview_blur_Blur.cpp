#include <com_glview_blur_Blur.h>
                  ...
                  )
{
    ...
    if (step == 1)
    {
        ...
        for(y = minY; y < maxY; y++)
        {
            ...
            src_ptr = src + stride * y;//src + w * y; // start of line (0,y)
            ...
            xp = radius;
            if (xp > wm) xp = wm;
            src_ptr = src + (xp + y * stride);//src + (xp + y * w); //   img.pix_ptr(xp, y);
    ...
    if (step == 2)
    {
        ...
        for(x = minX; x < maxX; x++)
        {
            ...
            for(i = 1; i <= radius; i++)
            {
                if(i <= hm) src_ptr += stride;//w; // +stride
            ...
            yp = radius;
            if (yp > hm) yp = hm;
            src_ptr = src + (x + yp * stride);//src + (x + yp * w); // img.pix_ptr(x, yp);
